...
write.csv(as.data.frame(svmTrained$results), paste0(path_lsvm, esi, comparison, "_lsvmCV.csv"), row.names = FALSE)
...
# svmModel$SV
# write.csv(as.data.frame(svmModel), paste0(exportAddr, "_lsvmModel.csv"), row.names = FALSE)
...
write.csv(
  cbind.data.frame(
    ...
    PredictedProbab = pred_prob[, 1], 
    predictedClass = pred
  ), 
  paste0(path_lsvm, esi, comparison, "_lsvmPredictions.csv"), 
  row.names = FALSE
)
...
png(paste0(path_lsvm, esi, comparison, "_lsvmROC.png"), width = 2000, height = 2000, res = 300)
print(
  ggplot(rocData, 
    aes(x = 1 - Specificity, 
        y = Sensitivity)) +
...
write.csv(rocData, paste0(path_lsvm, esi, comparison, "_lsvmROC.csv"))
write.csv(confMatrix_table, paste0(path_lsvm, esi, comparison, "_lsvmConfusionMatrix.csv"))
write.csv(confMatrix_metrics, paste0(path_lsvm, esi, comparison, "_lsvmMetrics.csv"))
